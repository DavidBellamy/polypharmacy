...
class GeneralConvWithBasis(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
        ...
        if self.skip_linear or self.in_channels != self.out_channels:
            assert self.basis_lin_self_wt is not None and self.basis_lin_self_biases is not None
            lin_self_wt = torch.matmul(self.basis_lin_self_wt,
    ...
    def message_basic(self, x_i: Tensor, x_j: Tensor):
        lin_msg_biases = torch.matmul(self.basis_lin_msg_biases,
                                      self.linear_combinations.t()).squeeze()
        lin_msg_wt = torch.matmul(self.basis_lin_msg_wt, self.linear_combinations.t()).squeeze()
...
class HeteroGAE(nn.Module):
    def __init__(self, hidden_dims, out_dim, node_types, edge_types,
                 decoder_2_relation, relation_2_decoder, num_bases=None, input_dim=None, dropout=0.5, device="cpu"):
        super().__init__()
        ...
        if self.num_bases:
            ...
            self.basis_lin_msg_biases = [nn.Parameter(torch.FloatTensor(hidden_dims[0], self.num_bases))]
            # basis_lin_msg_biases is a basis of the linear transformation for the bias
            self.basis_lin_self_wt = [
                nn.Parameter(torch.FloatTensor(input_dim['drug'], hidden_dims[0], self.num_bases))]
            # basis_lin_self_wt is a basis of the linear transformation for the self-connection
            self.basis_lin_self_biases = [nn.Parameter(torch.FloatTensor(hidden_dims[0], self.num_bases))]
            # basis_lin_self_biases is a basis of the linear transformation for the bias for self-connection
            self.linear_combinations = [nn.ParameterDict({
            ...
            nn.init.xavier_uniform_(self.basis_lin_msg_wt[0])  # initialize the basis_lin_msg_wt
            nn.init.xavier_uniform_(self.basis_lin_msg_biases[0])  # initialize the basis_lin_msg_biases
            nn.init.xavier_uniform_(self.basis_lin_self_wt[0])  # initialize the basis_lin_self_wt
            nn.init.xavier_uniform_(self.basis_lin_self_biases[0])  # initialize the basis_lin_self_biases
            for params in self.linear_combinations[0].values():  # initialize the linear_combinations
                nn.init.xavier_uniform_(params)
            for i, hidden_dim in enumerate(hidden_dims[1:]):  # instantiate the basis_lin_msg_wt, basis_lin_msg_biases,
                # basis_lin_self_wt, basis_lin_self_biases for the hidden layers
                self.basis_lin_msg_wt.append(
                    nn.Parameter(torch.FloatTensor(hidden_dims[i], hidden_dim, self.num_bases)))
                self.basis_lin_msg_biases.append(nn.Parameter(torch.FloatTensor(hidden_dim, self.num_bases)))
                self.basis_lin_self_wt.append(
                    nn.Parameter(torch.FloatTensor(hidden_dims[i], hidden_dim, self.num_bases)))
                self.basis_lin_self_biases.append(nn.Parameter(torch.FloatTensor(hidden_dim, self.num_bases)))
    ...
    def generate_hetero_conv_dict(self):
        ...
        for i in range(len(self.hidden_dims)):  # for each layer in the encoder
            D = {}

            for edge_type in self.edge_types:
                src, relation, dst = edge_type  # src is the source node type, dst is the destination node type
                if relation not in ["interact", "has_target", "get_target"]:
                    if self.num_bases is not None and src == 'drug' and dst == 'drug':
                        ...
                        D[edge_type] = GeneralConvWithBasis(in_channels, self.hidden_dims[i],
                                                            self.basis_lin_msg_wt[i], self.basis_lin_msg_biases[i],
                                                            self.linear_combinations[i][str(edge_type)],
                                                            self.basis_lin_self_wt[i], self.basis_lin_self_biases[i],
                                                            aggr="sum",
                                                            skip_linear=True, l2_normalize=True)
                    else:
                        D[edge_type] = GeneralConv((-1, -1), self.hidden_dims[i], aggr="sum", skip_linear=True,
                                                   l2_normalize=True)
                else:
                    D[edge_type] = GeneralConv((-1, -1), self.hidden_dims[i], aggr="sum", skip_linear=True,
                                               l2_normalize=True)
    ...
    def decode_all_relation(self, z_dict, edge_index_dict, sigmoid=False):
        ...
        for edge_type in self.edge_types:  # iterate over all edge types
            if edge_type not in edge_index_dict.keys():  # skip if edge type is not present in the graph
                continue
            src, relation, dst = edge_type  # unpack the edge type
            decoder_type = self.relation_2_decoder[relation]  # get the decoder type for the relation
            z = (z_dict[src], z_dict[dst])  # get the latent representations for the source and destination nodes
            output[relation] = self.decoder[decoder_type](z, edge_index_dict[edge_type], relation)  # decode
